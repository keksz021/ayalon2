...
class CMainPacketHeaderMap : public CNetworkPacketHeaderMap
{
	...
	public:
		CMainPacketHeaderMap()
		{
			...
			Set(HEADER_GC_WHISPER, CNetworkPacketHeaderMap::TPacketType(sizeof(TPacketGCWhisper), DYNAMIC_SIZE_PACKET));
			...
			Set(HEADER_GC_MARK_BLOCK, CNetworkPacketHeaderMap::TPacketType(0, DYNAMIC_SIZE_PACKET));
			Set(HEADER_GC_MARK_DIFF_DATA, CNetworkPacketHeaderMap::TPacketType(0, DYNAMIC_SIZE_PACKET));
			Set(HEADER_GC_MARK_IDXLIST, CNetworkPacketHeaderMap::TPacketType(0, DYNAMIC_SIZE_PACKET));
...
#endif
#ifdef ENABLE_ACHIEVEMENT_SYSTEM
			// Dinamikus, mert a subheader dönti el a további felépítést.
