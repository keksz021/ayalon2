...
DWORD CInstanceBase::__AttachEffectToArmours(std::string effectfilename)
{
...
void CInstanceBase::__AttachMetinEffect()
{
	if (!__IsExistMainInstance())
		return;
	...
	if (IsWarp() || IsObject() || IsFlag() || IsResource())
		return;

	__EffectContainer_AttachEffect(EFFECT_METIN);
}
#endif
...
void CInstanceBase::RefreshTextTail()
{
	...
	bool bHasAchievementTitle = false;

#ifdef ENABLE_ACHIEVEMENT_SYSTEM
	if (GetAchievementTitle() != 0) {
		std::string szTitleName;
		if (CAchievementSystem::Instance().GetTitleName(GetAchievementTitle(), szTitleName)) {
			// Achievement cím a "Title" slotba
			CPythonTextTail::Instance().AttachTitle(GetVirtualID(), szTitleName.c_str(), GetTitleColor());
			bHasAchievementTitle = true;

		}
	}
#endif

	// ALIGNMENT csak akkor, ha nincs achievement cím
	if (!bHasAchievementTitle)
	{
		...
		{
			auto it = g_TitleNameMap.find(iAlignmentGrade);
			if (it != g_TitleNameMap.end())
			{
				CPythonTextTail::Instance().AttachTitle(GetVirtualID(), it->second.c_str(), GetTitleColor());
			}
		}
	}

#ifdef ENABLE_TITLE_SYSTEM
	// A külön, fel/levehetõ “TitleSystem” mindig mehet a saját slotjára
	...
	{
		auto it2 = g_TitleSystemNameMap.find(iTitle);
		if (it2 != g_TitleSystemNameMap.end())
		{
			CPythonTextTail::Instance().AttachTitleSystem(GetVirtualID(), it2->second.c_str(), GetTitleSystemColor());
		}
	}
#endif
}
...
void CInstanceBase::__SetAffect(UINT eAffect, bool isVisible)
{
	...
	case AFFECT_INVISIBILITY:
		...
		{
			if (isVisible)
				m_GraphicThingInstance.BlendAlphaValue(0.0f, 0.0f);//m_GraphicThingInstance.BlendAlphaValue(0.5f, 1.0f); eredeti
			else
				m_GraphicThingInstance.BlendAlphaValue(0.0f, 0.0f); // 1.0f , 1.0f
			break;
		}
#endif
		if (isVisible)
...
)
{
	...
	if (ms_astAffectEffectAttachBone[eEftType].empty())
	{
		...
		if (IsPremium())
#ifdef ENABLE_AURA_COSTUME_SYSTEM
			return m_GraphicThingInstance.AttachEffectByID(0, NULL, ms_adwCRCAffectEffect[eEftType], NULL, dwSkillColor, fScale, (const D3DXVECTOR3*)NULL);
#else
...
#endif
#ifdef ENABLE_AURA_COSTUME_SYSTEM
			return m_GraphicThingInstance.AttachEffectByID(0, NULL, ms_adwCRCAffectEffect[eEftType], NULL, NULL, fScale, (const D3DXVECTOR3*)NULL);
#else
	...
	else
	{
		...
		else
		{
			...
			if (IsPremium())
#ifdef ENABLE_AURA_COSTUME_SYSTEM
				return m_GraphicThingInstance.AttachEffectByID(0, rstrBoneName.c_str(), ms_adwCRCAffectEffect[eEftType], NULL, dwSkillColor, fScale, (const D3DXVECTOR3*)NULL);
#else
...
#endif
#ifdef ENABLE_AURA_COSTUME_SYSTEM
				return m_GraphicThingInstance.AttachEffectByID(0, rstrBoneName.c_str(), ms_adwCRCAffectEffect[eEftType], NULL, NULL, fScale, (const D3DXVECTOR3*)NULL);
#else
