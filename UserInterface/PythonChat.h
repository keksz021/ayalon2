...
class CPythonChat : public CSingleton<CPythonChat>, public IAbstractChat
{
	public:
		...
		typedef struct SEmoticonsRecentChat
		{
			std::string codigo;
			int count;
			int check;
		} TEmoticonsRecentChat;

		typedef std::map<std::string, TEmoticonsRecentChat> TEmoticonsRecentChatVector;
		typedef std::vector<TEmoticonsRecentChat> TEmoticonsRecentChatSet;
#endif
	...
	public:
		...
		void SaveEmojisChat (const char* c_szEmojisChat);
#endif
...
#ifdef ENABLE_EMOTICONS_SYSTEM
		TEmoticonsRecentChatVector			m_EmoticonsRecentChatVector;
		TEmoticonsRecentChatSet				m_EmoticonsRecentChatSet;
#endif
