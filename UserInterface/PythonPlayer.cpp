...
void CPythonPlayer::SendClickItemPacket(DWORD dwIID)
{
	...
	// --- anti-spam ugyanarra az itemre (instant marad) ---
	static DWORD s_lastItem = 0, s_lastTime = 0;
	DWORD now = ELTimer_GetMSec();
	if (dwIID == s_lastItem && now - s_lastTime < 250) // ~0.25s ablak
		return;
	s_lastItem = dwIID;
	s_lastTime = now;
	...
	if (now < s_dwNextTCPTime)
		return;
	s_dwNextTCPTime = now + 500;
#endif
	...
	if (c_szOwnerName[0] && 0 != strcmp(c_szOwnerName, GetName()))
	{
		CItemData* pItemData;
		...
		{
			// üzenet throttle, hogy ne spam-eljen
			static DWORD s_lastMsg = 0;
			if (now - s_lastMsg > 800) {
	...
	CPythonNetworkStream::Instance().SendItemPickUpPacket(dwIID);
}
...
CPythonPlayer::CPythonPlayer(void)
{
	...
	m_kMap_dwAffectIndexToSkillIndex.insert(std::make_pair(int(CInstanceBase::AFFECT_HOSIN_ITEM), 163));
	m_kMap_dwAffectIndexToSkillIndex.insert(std::make_pair(int(CInstanceBase::AFFECT_BOHO_ITEM), 164));
	m_kMap_dwAffectIndexToSkillIndex.insert(std::make_pair(int(CInstanceBase::AFFECT_GICHEON_ITEM), 165));
	m_kMap_dwAffectIndexToSkillIndex.insert(std::make_pair(int(CInstanceBase::AFFECT_KWAESOK_ITEM), 166));
	m_kMap_dwAffectIndexToSkillIndex.insert(std::make_pair(int(CInstanceBase::AFFECT_JEUNGRYEOK_ITEM), 167));
#endif
...
#ifdef ENABLE_METIN_QUEUE
void CPythonPlayer::CancelQueue()
{
	__ClearTarget();
	__ClearAutoAttackTargetActorID();
	...
	for (auto& i : m_queueList)
	{
		CInstanceBase* victim = NEW_FindActorPtr(i);
		if (victim && !victim->IsDead())
			victim->SetMetinQueueEffect(true);
	}

	m_queueList.clear();
}

void CPythonPlayer::ClearQueueVector(BYTE idx)
{
	auto temp = m_queueList;
	m_queueList.clear();
	for (int i = 0; i < temp.size(); i++)
	{
		...
		m_queueList.push_back(temp[i]);
	}
}

void CPythonPlayer::UpdateQueue()
{
	CInstanceBase* pInstance = CPythonCharacterManager::Instance().GetMainInstancePtr();
	if (!pInstance)
		return;

	bool goNextTarget{ true };
	...
	if (pInstance->IsAttacking())
	{
		goNextTarget = false;
	}

	CInstanceBase* victim = NEW_FindActorPtr(m_attackedStoneVID);


	// TODO: Go Nearest Metinstone
	// 
	//const float fDistance = pInstance->NEW_GetDistanceFromDestInstance(*victim); 
	//std::sort(m_queueList.begin(), m_queueList.end(), isNearerVID);

	if (victim && !victim->IsDead() && goNextTarget)
	{
		if (GetTargetVID() != m_attackedStoneVID || m_dwAutoAttackTargetVID != m_attackedStoneVID)
		{
			__SetTargetVID(m_attackedStoneVID);
			__SetAutoAttackTargetActorID(m_attackedStoneVID);
		}
		return;
	}
	...
	{
		victim = NEW_FindActorPtr(m_queueList[i]);

		if (!victim || (victim && victim->IsDead()))
		{
			m_attackedStoneVID = 0;
			ClearQueueVector(i);
			return;
		...
		m_attackedStoneVID = m_queueList[i];
		break;
	}
...
void CPythonPlayer::AddQueueList(CInstanceBase* victim)
{
	if (!victim)
		return;
	...
	if (m_attackedStoneVID == victim->GetVirtualID())
	{
		m_attackedStoneVID = 0;
	}
	...
	for (int i = 0; i < m_queueList.size(); i++)
	{
		if (m_queueList[i] == victim->GetVirtualID())
		{
			ClearQueueVector(i);
			victim->SetMetinQueueEffect(true);
	...
	if (pInstance->IsMetinQueue())
	{
		if (m_queueList.size() >= 6)
			return;
	}
	else
	{
		if (m_queueList.size() >= 3)
			return;
	}

	m_queueList.push_back(victim->GetVirtualID());
	victim->SetMetinQueueEffect();
	m_isQueue = true;
}
#endif