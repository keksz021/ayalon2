...
void SNetworkActorData::__copy__(const SNetworkActorData& src)
{
	...
	m_dwAchievementTitle = src.m_dwAchievementTitle;
#endif
...
void CNetworkActorManager::__RemoveDynamicActors()
{
	...
	for( int i = 0; i < dwCharacterVIDList.size(); ++i )
	{
		...
		|| rkActorEach->IsStone() || rkActorEach->IsPet() || rkActorEach->IsMount()
#ifdef ENABLE_GROWTH_PET_SYSTEM
...
#endif
#ifdef ENABLE_ACHIEVEMENT_SYSTEM
	kCreateData.m_dwAchievementTitle = rkNetActorData.m_dwAchievementTitle;
#endif
...
void CNetworkActorManager::UpdateActor(const SNetworkUpdateActorData& c_rkNetUpdateActorData)
{
	...
	if (pkInstFind)
	{
		...
		pkInstFind->SetLevel(c_rkNetUpdateActorData.m_dwLevel);	//level fix
		pkInstFind->SetPKMode(c_rkNetUpdateActorData.m_byPKMode);
#ifdef ENABLE_TITLE_SYSTEM
		pkInstFind->SetTitleSystem(c_rkNetUpdateActorData.m_iTitleID);
#endif
#ifdef ENABLE_ACHIEVEMENT_SYSTEM
		pkInstFind->SetAchievementTitle(c_rkNetUpdateActorData.m_dwAchievementTitle);
#endif
...
#endif
#ifdef ENABLE_ACHIEVEMENT_SYSTEM
	rkNetActorData.m_dwAchievementTitle = c_rkNetUpdateActorData.m_dwAchievementTitle;
#endif
#ifdef ENABLE_MULTI_LANGUAGE_SYSTEM
