...
bool LoadLocaleData(const char *localePath)
{
	...
	snprintf(szLocaleQuestList, sizeof(szLocaleQuestList), "%s/locale_quest.txt", localePath);
	...
	{
		TraceError("LoadLocaleData - LoadLocaleQuestVnum(%s) Error", szLocaleStringList);
		return false;
	}
...
{
	// 0. embedded browser / web UI lezárása
	WebBrowser_Destroy();

	// 1. UI és ablakrendszer bezárása
	m_kWndMgr.Destroy();
	...
	// 2. Hangrendszer és input/evt rendszerek
	m_SoundManager.Destroy();

	// 3. Effektek, karakterek, itemek
	m_kEftMgr.Destroy();
	m_kChrMgr.Destroy();
	...
	// 4. Térkép / háttér / fény / projectile rendszerek
	m_pyBackground.Destroy();
	m_LightManager.Destroy();
	m_FlyingManager.Destroy();

	// 5. Render targetek és resource manager
	m_kRenderTargetManager.Destroy();
	m_pyRes.Destroy();

	// 6. Granny deform buffer
	GrannyDestroySharedDeformBuffer();

	// 7. Grafikus objektum poolok és textúrák
	...
	// 8. A tényleges grafikus device-ek bontása (FONTOS: csak ezután!)
	m_pyGraphic.Destroy();
	m_grpDevice.Destroy();

	// 9. Guild mark / hálózat / opcionális integrációk
	m_kGuildMarkDownloader.Disconnect();
#if defined(ENABLE_DISCORD_RPC)
	m_pyNetworkStream.Discord_Close();
#endif

	// 10. Attribute / text loader rendszerek
	CAttributeInstance::DestroySystem();
	CTextFileLoader::DestroySystem();

	// 11. Cursorok és alkalmazásszintû UI
	DestroyCursors();
	CMSApplication::Destroy();

	// 12. Anyaghoz kötött globális erõforrások
	CGrannyMaterial::DestroySphereMap();

	// 13. StickyKeys visszaállítás Windowsnak
	STICKYKEYS sStickKeys;
	memset(&sStickKeys, 0, sizeof(sStickKeys));
	sStickKeys.cbSize = sizeof(sStickKeys);
