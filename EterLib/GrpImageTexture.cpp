...
bool CGraphicImageTexture::CreateFromMemoryFile(UINT bufSize, const void* c_pvBuf, D3DFORMAT d3dFmt, DWORD dwFilter)
{
	...
	{
		TraceError("CreateFromMemoryFile: Invalid buffer (NULL or zero size)");
		return false;
	}
	...
	// DDS betöltés, csak ha nem TGA (külön kezelendő)
	if (!strstr(m_stFileName.c_str(), ".tga") && image.LoadHeaderFromMemory((const BYTE*)c_pvBuf))
	{
		...
		{
			TraceError("CreateFromMemoryFile: DDS load failed (%s)", m_stFileName.c_str());
			return false;
		}
	}
	else
	{
		...
		if (FAILED(hr) || !m_lpd3dTexture)
		{
			TraceError("CreateFromMemoryFile: Cannot create texture (file=%s, size=%u, hr=0x%08x)", m_stFileName.c_str(), bufSize, hr);
			m_lpd3dTexture = NULL;
			return false;
		}
		...
		// Formátum konverzió, ha szükséges
		D3DFORMAT format = imageInfo.Format;
		switch (imageInfo.Format)
		{
		case D3DFMT_A8R8G8B8: format = D3DFMT_A4R4G4B4; break;
		case D3DFMT_X8R8G8B8:
		case D3DFMT_R8G8B8:   format = D3DFMT_A1R5G5B5; break;
		}
		...
		if (IsLowTextureMemory() && (uTexBias || format != imageInfo.Format))
		{
			...
			{
				TraceError("CreateFromMemoryFile: D3DXCreateTexture (rescale) failed file=%s", m_stFileName.c_str());
			}
		}
	}
	...
	if (!m_lpd3dTexture)
	{
		TraceError("CreateFromMemoryFile: NULL texture pointer after load (file=%s)", m_stFileName.c_str());
		return false;
	}
	...
	if (!bDDS)
	{
		...
		{
			TraceError("CreateFromMemoryFile: Invalid texture size (file=%s)", m_stFileName.c_str());
			m_lpd3dTexture->Release();
			m_lpd3dTexture = NULL;
			return false;
		}
