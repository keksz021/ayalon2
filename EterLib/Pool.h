	...
	public:
		...
		virtual ~CDynamicPool()
		{
			...
			if (!m_kVct_pkData.empty())
				OutputDebugStringA("[POOL] CDynamicPool dtor: soft-clear (skip delete)\n");
#endif
			// Soft clear: ne hívd Delete()-et, csak ürítsd a vektorokat
			m_kVct_pkFree.clear();
			m_kVct_pkData.clear();
		}
		...
		void Destroy()
		{
			if (m_kVct_pkData.empty() && m_kVct_pkFree.empty())
				return;
...
class CDynamicPoolEx
{
	public:
		...
		void Destroy()
		{
			...
			if (m_kVct_pkData.empty() && m_kVct_pkFree.empty()) // <<< ADD
				return;
