...
SKeyTimer				CInputKeyboard::m_KeyTimers[256];
...
// Alt + F4¸¦ À§ÇØ ºñµ¶Á¡ ¸ğµå·Î - [levites]
...
void CInputKeyboard::UpdateKeyboard()
{
	if (!ms_lpKeyboard)
		return;
	// Detect if window is minimized or not focused (sima kikattintós cucc)
	if (GetForegroundWindow() != g_hWnd || IsIconic(g_hWnd))
	{
		...
		{
			if (IsPressed(i))
				KeyUp(i); // Ez egyéni feldolgozás függvénye, nálad lehet más is
		}
	
		// Clear current state to prevent ghost inputs
		memset(ms_diks, 0, sizeof(ms_diks));
		return;
	}
	...
	if (FAILED(hr))
	{
		...
		// ÇöÀç ¾îÇÃ¸®ÄÉÀÌ¼ÇÀÌ ºñÈ°¼ºÈ­ µÇ¾î ÀÖ¾î ÀÔ·ÂÀ» ¹ŞÀ» ¼ö ¾ø´Ù.
