...
#endif
// --- Debug jelzés globális inithez ---
struct __DbgOnce_GrpText {
	__DbgOnce_GrpText() { OutputDebugStringA("DEBUG: GrpTextInstance global init\n"); }
};
static __DbgOnce_GrpText __dbg_once_grptext;
// --------------------------------------
		...
		else 
		{
				...
				for (i = wArabicTextLen - 1 ; i >= 0; --i)
				{
					...
					if (isEnglish)
					{
						if (Arabic_IsInSymbol(wArabicChar) && (
								(i == 0) ||
								(i > 0 && 
									!(Arabic_HasPresentation(wArabicText, i - 1) || Arabic_IsInPresentation(wArabicText[i + 1]))  && //¾Õ±ÛÀÚ, µÞ±ÛÀÚ°¡ ¾Æ¶ø¾î ¾Æ´Ô.
									wArabicText[i+1] != '|'
								) ||
								wArabicText[i] == '|'
							))
			...
			else
			{
				...
				for (int i = 0; i < wTextLen; )
				{
					...
					{
						char retBuf[1024];
						int retLen = Ymir_WideCharToMultiByte(GetDefaultCodePage(), 0, emoticonBuffer.c_str(), emoticonBuffer.length(), retBuf, sizeof(retBuf) - 1, NULL, NULL);
						retBuf[retLen] = '\0';
