...
#define DO1(buf, i)   crc = CRCTable[(crc ^ (unsigned char)buf[i]) & 0xff] ^ (crc >> 8)
...
#define DO1CI(buf, i)  crc = CRCTable[(crc ^ (unsigned char)UPPER(buf[i])) & 0xff] ^ (crc >> 8)
...
{
    // NINCS malloc/strcpy/strcat – nincs leak és nincs pointer-eldobás.
    std::string data;
    if (name && *name)
    {
        if (len == 0 && buf)
            len = std::strlen(buf);
        data.reserve((name ? std::strlen(name) : 0) + len);
    ...
    const char* p = data.empty() ? "" : data.c_str();
    size_t n = data.size();
...
DWORD GetHFILECRC32(HANDLE hFile)
{
    ...
    HANDLE hFM = CreateFileMapping(hFile, NULL, PAGE_READONLY, 0, dataOffset + mapSize, NULL);
    if (hFM)
    {
        LPVOID lpMapData = MapViewOfFile(hFM, FILE_MAP_READ, 0, dwFileMapStart, dwMapViewSize);
        if (lpMapData)
        {
            dwRetCRC32 = GetCRC32(reinterpret_cast<const char*>(lpMapData), dwFileSize);
...
DWORD GetFileSize(const char* c_szFileName)
{
    ...
    DWORD dwSize = ::GetFileSize(hFile, NULL); // WinAPI verzió – kvalifikálva!
    CloseHandle(hFile);
    return dwSize;
}
