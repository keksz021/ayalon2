...
namespace UI
{
	...
	BOOL CWindow::OnMouseLeftButtonUp()
	{
		...
		return TRUE; // NOTE : ButtonUpŔş żążÜ·Î ą«Á¶°Ç TRUE
	}
	...
	void CImageBox::OnDestroyInstance()
	{
		...
		// 1) Törd a kapcsolatot a kép és az instance között,
		//    hogy az instance Destroy() ne fogdosson már resource-ot.
		m_pImageInstance->SetImagePointer(NULL);

		// 2) Most már biztonságos a pool felé visszaadni.
	...
	BOOL CImageBox::LoadImage(const char* c_szFileName)
	{
		...
		// Ha ugyanazt akarjuk újratölteni, semmit ne csináljunk
		if (!m_strFileName.empty() && !_stricmp(m_strFileName.c_str(), c_szFileName))
			return TRUE;

		// Előző példány biztonságos felszabadítása
		if (m_pImageInstance)
		{
			m_pImageInstance->Destroy();  // saját textúra release
			delete m_pImageInstance;
			m_pImageInstance = NULL;
		}
		...
		if (!pResource)
		{
			TraceError("CImageBox::LoadImage - resource not found: %s", c_szFileName);
			return FALSE;
		}

		if (!pResource->IsType(CGraphicImage::Type()))
		{
			TraceError("CImageBox::LoadImage - invalid type: %s", c_szFileName);
			return FALSE;
		}
		...
		if (!pImage)
		{
			TraceError("CImageBox::LoadImage - invalid image pointer: %s", c_szFileName);
			return FALSE;
		}
		...
		{
			TraceError("CImageBox::LoadImage - empty image: %s", c_szFileName);
			return FALSE;
		}
