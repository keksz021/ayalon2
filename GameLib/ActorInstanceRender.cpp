...
void CActorInstance::RenderCollisionData()
{
	...
	if (false)
	{
		...
		screen.RenderSphere(NULL, object_center.x, object_center.y, object_center.z, sphere_radius, D3DFILL_WIREFRAME);
	}
	...
	for (const TCollisionPointInstance& point : m_DefendingPointInstanceList)
	{
		for (const CDynamicSphereInstance& collision : point.SphereInstanceVector)
		{
			...
			screen.RenderSphere(NULL, collision.v3Position.x, collision.v3Position.y, collision.v3Position.z, collision.fRadius, D3DFILL_WIREFRAME);
		}
	}
